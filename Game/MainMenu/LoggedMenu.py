...
class PopUpLogOut:
    ...
    def draw(self, interfaz):
        ...
        x_popup = w // 2 - BK.get_width() // 2
        y_popup = h // 2 - BK.get_height() // 2
        interfaz.blit(BK, (x_popup, y_popup))
        
        mouse_pos = pygame.mouse.get_pos()
        size_button = self.scale(204, 33, w, h)
        
        # Actualizar dimensiones de los botones
        self.YesButton.width, self.YesButton.height = int(size_button[0]), int(size_button[1])
        self.NoButton.width, self.NoButton.height = int(size_button[0]), int(size_button[1])

        # Posicionamiento
        self.NoButton.x = x_popup + self.scale(36, 0, w, h)[0]
        self.NoButton.y = y_popup + self.scale(0, 170, w, h)[1]
        self.YesButton.x = x_popup + self.scale(252, 0, w, h)[0]
        self.YesButton.y = y_popup + self.scale(0, 170, w, h)[1]

        for button in [self.YesButton, self.NoButton]:
            if button.is_hovered(mouse_pos):
                size = (int(button.width * 1.1), int(button.height * 1.1))
